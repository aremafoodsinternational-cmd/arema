// sync-blogs.js — Sync 5 new blog articles to Supabase
// Run: node sync-blogs.js

const { createClient } = require('@supabase/supabase-js');

const SUPABASE_URL = 'https://ktfhecsftxqdsuidpfht.supabase.co';
const SUPABASE_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imt0ZmhlY3NmdHhxZHN1aWRwZmh0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODIzMTUxMjcsImV4cCI6MjA5Nzg5MTEyN30.HA10Qsh_BG7uRmmmw3ymVtlTjyDG0BaoS_bLIiLIr58';
...
const BLOG_ARTICLES = [
  {
    ...
    translation: {
      ...
      excerpt: 'In our childhood, one of the most significant sights and ways of life in our countryside was the vast paddy fields—spread out like a lush green carpet, the true source of rice and grain.',
      body: [
  ...
  {
    base: { id: 'premium-matta-rice-dominating-global-markets', image_url: '/images/tl-05.png' },
    translation: {
      blog_id: 'premium-matta-rice-dominating-global-markets',
      lang: 'en',
      category: 'Trade',
      readTime: '5 min read',
      date: 'June 2026',
      title: 'Taste the Tradition of Kerala with Every Grain : Why Premium Matta Rice is Dominating Global Markets',
      excerpt: 'The global demand for authentic, nutrient-rich ethnic foods is reaching an all-time high. Sourcing authentic regional staples like Kerala Matta rice is the fastest way to build customer loyalty.',
      body: [
  ...
  {
    base: { id: 'why-matta-doesnt-taste-like-90s', image_url: '/images/matta-grain-hands.png' },
    translation: {
      blog_id: 'why-matta-doesnt-taste-like-90s',
      lang: 'en',
      category: 'Quality',
      readTime: '6 min read',
      date: 'June 2026',
      title: "Why doesn't Matta taste like the 90's anymore ? and how we brought it back?",
      excerpt: '"Pazhaya aa taste ippo kittunnilla." If you\'ve heard your parents say this, or caught yourself thinking it after a bland sadhya outside Kerala, you\'re not imagining things. The Palakkadan Matta of the 90s has quietly vanished.',
      body: [
  ...
  {
    base: { id: 'meet-bharathapuzha-farmers', image_url: '/images/tl-01.png' },
    translation: {
      blog_id: 'meet-bharathapuzha-farmers',
      lang: 'en',
      category: 'Farming',
      readTime: '5 min read',
      date: 'June 2026',
      title: 'Meet the Bharathapuzha Farmers Behind Our Arema Rice',
      excerpt: 'Every pack of Arema GI tagged Palakkadan Matta starts with a farmer, not a factory. Across the Bharathapuzha belt, generations of farmers still grow paddy the way GI tag demands.',
      body: [
  ...
  {
    base: { id: 'three-myths-about-palakkadan-matta', image_url: '/images/tl-02.png' },
    translation: {
      blog_id: 'three-myths-about-palakkadan-matta',
      lang: 'en',
      category: 'Quality',
      readTime: '5 min read',
      date: 'June 2026',
      title: 'Three Myths about Palakkadan Matta that even Malayalis believe',
      excerpt: 'Not all red rice is Palakkadan Matta — and half of Kerala still gets fooled. We debunk the common myths surrounding this famous grain.',
      body: [
...
async function syncBlogs() {
  console.log('🔄 Syncing 5 blog articles to Supabase...\n');

  // First, get all existing blog IDs
  const { data: existingBlogs } = await supabase.from('blogs').select('id');
  const existingIds = (existingBlogs || []).map(b => b.id);
  const newIds = BLOG_ARTICLES.map(a => a.base.id);
  
  // Delete old blog entries that are not in the new set
  const toDelete = existingIds.filter(id => !newIds.includes(id));
  if (toDelete.length > 0) {
    console.log(`🗑️  Removing ${toDelete.length} old blog(s): ${toDelete.join(', ')}`);
    for (const id of toDelete) {
      await supabase.from('blog_translations').delete().eq('blog_id', id);
  ...
  for (const article of BLOG_ARTICLES) {
    const { base, translation } = article;
    
    // Upsert base blog record
    const { error: baseErr } = await supabase
      .from('blogs')
      .upsert(base, { onConflict: 'id' });
    
    if (baseErr) {
      console.error(`❌ Failed to upsert blog "${base.id}": ${baseErr.message}`);
      continue;
    }
    ...
    const { error: transErr } = await supabase
      ...
      .upsert(translation, { onConflict: 'blog_id,lang' });
    
    if (transErr) {
      console.error(`❌ Failed to upsert translation for "${base.id}": ${transErr.message}`);
      continue;
    }
    
    console.log(`✅ "${base.id}" synced successfully.`);
  }
